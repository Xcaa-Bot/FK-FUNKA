(async () => {
     switch(true) {
         case /^setpp(gro?up|gc)$/i.test(command): {
             if (!isGroup) return reply('Fitur Khusus Group')
             if (!isBotAdmin) return reply('Jadikan Bot Admin Terlebih Dahulu')
          ...
          case /^promote$/i.test(command): {
              ...
              if (!mem[0]) return reply(`Contoh : ${prefix + command} Tag/Reply`)
